...
import { readableStreamToString, XkcdError, parseIfJson } from "./error";
        ...
        if (e instanceof AxiosError && e.isAxiosError) {
            try {
                const responseBody =
                    ...
                    : e.response?.data
                throw new XkcdError(e, parseIfJson(responseBody))
            } catch (innerError) {
                if (innerError instanceof ReferenceError) {
                    ...
                    throw new XkcdError(e, e.response?.data)
                }
                if (innerError instanceof XkcdError) {
                    // Got "XkcdError" from the above try block
                    throw innerError;
                }
